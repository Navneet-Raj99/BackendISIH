...
if __name__ == '__main__':
    ...
    def store_in_mongodb(data):
        ...
        encrypted_data=Latestencryption(data_string,"mp_-MzjYa3yqbfi-LKVM7W_SsZ8tKsVATNBJQvko-vM=")
    ...
    def delete_data_from_csv(file_path):
        with open(file_path, 'w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
            existing_data = list(csv_reader)
            remaining_data = [row for row in existing_data if row not in data_to_delete]
            with open(file_path, 'w', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
    ...
    def upload_to_server(data):
        try:
            for row in data:
                ...
                encryptedData=Latestencryption(data_string, "mp_-MzjYa3yqbfi-LKVM7W_SsZ8tKsVATNBJQvko-vM=")
                store_in_mongodb(row)
                client.publish("dashboard/vehicle", payload=encryptedData, qos=0)
        except Exception as e:
            print(f"Error uploading to server: {e}")


    def continues_running_function(file_path):
        while True:
            ...
            if data:
                ...
                if check_internet_connection():
                    ...
                    if upload_success:
                        delete_data_from_csv(data)
                else:
                    print("No internet connection. Retrying in 10 seconds.")
                    time.sleep(10)
            else:
                print("No data available. Waiting for new data.")
                time.sleep(10)  # Adjust as needed based on your requirements
            time.sleep(10)
    ...
    def mqtt_thread():
        while True:
            if ser2.in_waiting > 0 and ser.in_waiting>0:
                ...
                print(newline)
                encryptedData=Latestencryption(newline, "mp_-MzjYa3yqbfi-LKVM7W_SsZ8tKsVATNBJQvko-vM=")
                print(encryptedData)
                continues_running_function(os.path(os.getcwd(),"OFFLINECSV","latest.csv"));
